...
import { Send, Search, Menu, MessageSquare } from "lucide-react";
...
import {
  ...
  useWidgetCreateRoom,
  useWidgetMessages,
  useWidgetCommunities,
} from "./hooks";
import type { Message, User, Community } from "@/types";

export function ChatWidget() {
  const { socket, email } = useWidgetContext();
  ...
  const { data: profile, isLoading: profileLoading } = useWidgetProfile();
  const { data: users = [], isLoading: usersLoading } = useWidgetUserList();
  const { data: communities = [] } = useWidgetCommunities();
  const { data: fetchedMessages } = useWidgetMessages(roomId);
  const createRoom = useWidgetCreateRoom();
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-background text-foreground">
      ...
      <aside
        className={`
          absolute inset-y-0 left-0 z-30 w-72 border-r bg-card transition-transform duration-200
      ...
      >
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center gap-3 border-b p-4">
            <Avatar src={profile?.img} alt={profile?.username} size="md" />
            ...
            {communities.length === 0 ? (
              ...
              communities.map((c: Community) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-accent"
                >
                  <Avatar fallback={c.name.charAt(0)} alt={c.name} size="md" />
                  <div className="min-w-0 flex-1">
      ...
      <main className="flex flex-1 flex-col">
        ...
        <header className="flex items-center gap-3 border-b px-4 py-3">
          ...
          {activeUser ? (
            <>
              <Avatar src={activeUser.img} alt={activeUser.username} size="md" />
              <div>
